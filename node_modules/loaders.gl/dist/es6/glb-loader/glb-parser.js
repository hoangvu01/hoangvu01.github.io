...
import assert from '../common/loader-utils/assert'; // glTF CONSTANTS
...
export default class GLBParser {
  ...
  parse(options = {}) {
    const _this$_parseBinary = this._parseBinary(this.glbArrayBuffer, options),
  ...
  parseWithMetadata(options = {}) {
    const _this$_parseBinary2 = this._parseBinary(this.glbArrayBuffer, options),
  ...
  _getApplicationJSON(json, options) {
    const jsonKey = options.jsonKey || 'json'; // Create glTF metadata object, with deleted application json key
  ...
  _parseBinary(options = {}) {
    ...
    assert(valid, `JSON chunk format ${jsonChunkFormat}`); // Create a "view" of the binary encoded JSON data

    const jsonChunkOffset = GLB_FILE_HEADER_SIZE + GLB_CHUNK_HEADER_SIZE; // First headers: 20 bytes

    const jsonChunk = new Uint8Array(this.glbArrayBuffer, jsonChunkOffset, jsonChunkLength); // Decode the JSON binary array into clear text

    const textDecoder = new TextDecoder('utf8');
    const jsonText = textDecoder.decode(jsonChunk); // Parse the JSON text into a JavaScript data structure

    const json = JSON.parse(jsonText); // TODO - BIN chunk can be optional
