...
export function parsePLY(data, options = {}) {
  ...
  if (faceNormal && !result.face.normals || vertexNormal && !result.vertex.nx) {
    const _generateNormals = generateNormals(result, flip),
...
export function generateNormals({
  ...
  triangles.forEach(vertices => {
    ...
    const v0 = vertices[0];
    const v1 = vertices[1];
    const v2 = vertices[2]; // get edge vectors of each triganle
    ...
    const x1 = x[v1] - x[v0];
    const y1 = y[v1] - y[v0];
    const z1 = z[v1] - z[v0]; // get cross-product betwee the two edge vectors

    let nx = y0 * z1 - z0 * y1;
    let ny = z0 * x1 - x0 * z1;
    let nz = x0 * y1 - y0 * x1; // sum per-triangle normal to adjacent vertex
