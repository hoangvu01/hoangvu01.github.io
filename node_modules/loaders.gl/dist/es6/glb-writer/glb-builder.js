...
export default class GLBBuilder {
  ...
  copyToArrayBuffer(arrayBuffer, byteOffset) {
    for (const _ref of this.parts) {
      const offset = _ref.offset;
      const contents = _ref.contents;
  ...
  _packBinaryChunk() {
    ...
    const targetArray = new Uint8Array(arrayBuffer); // Copy each array into

    let byteOffset = 0;

    for (let i = 0; i < this.sourceBuffers.length; i++) {
      const sourceBuffer = this.sourceBuffers[i];
      const byteLength = sourceBuffer.byteLength; // Pack buffer onto the big target array
  ...
  _createGlbBuffer(appJson, options = {}) {
    ...
    const jsonChunk = convertObjectToJsonChunk(this.json); // As body is 4-byte aligned, the scene length must be padded to have a multiple of 4.

    const jsonChunkLength = padTo4Bytes(jsonChunk.byteLength);
    const binChunkOffset = jsonChunkLength + jsonChunkOffset;
    const fileLength = binChunkOffset + GLB_CHUNK_HEADER_SIZE + padTo4Bytes(binChunk.byteLength); // Length is know, we can create the GLB memory buffer!

    const glbArrayBuffer = new ArrayBuffer(fileLength);
    const dataView = new DataView(glbArrayBuffer); // GLB Header
