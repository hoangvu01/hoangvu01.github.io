...
export default class KMLParser {
  ...
  static ParseDomDocuments(xmlDom) {
    ...
    for (let i = 0; i < docsDomNodes.length; i++) {
      ...
      const docsData = Object.assign({}, KMLParser.ParseDomDoc(node), KMLParser.ParseNonSpatialDomData(node, {}));
  ...
  static ParseDomLinks(xmlDom) {
    ...
    for (let i = 0; i < linkDomNodes.length; i++) {
      ...
      const linkData = Object.assign({}, KMLParser.ParseDomLink(node), KMLParser.ParseNonSpatialDomData(node, {}));
  ...
  static ParseDomItems(xmlDom, tag) {
    ...
    for (let i = 0; i < markerDomNodes.length; i++) {
      ...
      const parent = node.parentNode.nodeName === 'Placemark' ? node.parentNode : node.parentNode.parentNode.nodeName === 'Placemark' ? node.parentNode.parentNode : null;
  ...
  static RGBColorToKML(rgb, opacity) {
    ...
    const kmlStr = opacityStr.substring(0, 2) + '' + colorStr.substring(4, 6) + colorStr.substring(2, 4) + colorStr.substring(0, 2);
    return kmlStr;
  }
