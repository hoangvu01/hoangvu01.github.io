...
function normalizeKML(kml) {
  ...
  for (var key in kml) {
    ...
    try {
      for (var _iterator = kml[key][Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function normalizeKMLItem(item) {
  if (item.coordinates && item.coordinates.length) {
    if (Array.isArray(item.coordinates[0])) {
      item.coordinates = item.coordinates.map(function (_ref) {
      ...
      var _item$coordinates = _slicedToArray(item.coordinates, 3),
