...
(function (scope) {
  ...
  var msgIndex = 0;
  var waitHandlers = {}; // This method is scope-wide since the nacl module uses this fuction to notify
  // us of events

  scope.handleMessage = function (message_event) {
    var msg = message_event.data;
    var resolver = waitHandlers[msg.id];
    delete waitHandlers[msg.id]; // call the callback in a separate context, make sure we've cleaned our
  ...
  var doDataExchange = function doDataExchange(cmd, callback) {
  ...
  var LASLoader = function LASLoader(arraybuffer) {
  ...
  LASLoader.prototype.readData = function (count, offset, skip) {
    ...
    return new Promise(function (res, rej) {
      setTimeout(function () {
        if (!o.header) return rej(new Error('Cannot start reading data till a header request is issued'));
        var start;
  ...
  var LAZLoader = function LAZLoader(arraybuffer) {
    this.arraybuffer = arraybuffer;
  ...
  var LASFile = function LASFile(arraybuffer) {
    this.arraybuffer = arraybuffer;
    this.determineVersion();
    if (this.version > 13) throw new Error('Only file versions <= 1.3 are supported at this time');
    this.determineFormat();
    if (pointFormatReaders[this.formatId] === undefined) throw new Error('The point format ID is not supported');
  ...
  var LASDecoder = function LASDecoder(buffer, len, header) {
    this.arrayb = buffer;
...
//# sourceMappingURL=laslaz.js.map